...
def weighted_kmeans(data, K=10, max_iter=50, tol=1e-4):
    ...
    for iteration in range(max_iter):
        ...
        distances = torch.cdist(data, centroids[iteration])  # Calculate pairwise distances between data points and centroids
        labels = torch.argmin(distances, dim=1)  # Assign the closest centroid to each node
        weights = torch.tensor(calculate_weights_as_angles(data, centroids[iteration], labels.numpy()), dtype=torch.float32)
        ...
        new_centroids = torch.stack([
            weighted_x[labels == i].sum(dim=0) / weights[labels == i].sum() if (labels == i).sum() > 0 else centroids[iteration][i]
            for i in range(K)
        ])
