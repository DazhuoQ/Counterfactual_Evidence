...
def weighted_kmeans_wo_WC(data, K=10, max_iter=50, tol=1e-4):
    ...
    for iteration in range(max_iter):
        ...
        # new_centroids = data[torch.randint(0, num_nodes, (K,))]
        new_centroids = torch.stack([
            data[labels == i].mean(dim=0) if (labels == i).sum() > 0 else centroids[0][i]
            for i in range(K)
        ])
